...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while (i < self.numParticles):
            particle_list.append(self.legalPositions[i % len(self.legalPositions)])
            i += 1
    ...
    def observe(self, observation, gameState):
        ...
        for particle in self.legalPositions:
            true_distance = util.manhattanDistance(particle, pacmanPosition)
            beliefs[particle] = emissionModel[true_distance] * belief_distribution[particle] 

        if sum(beliefs.values()) == 0:
            self.initializeUniformly(gameState)
        else:
            for j in range(self.numParticles):
                self.particles[j] = util.sample(beliefs)
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            new_list.append(util.sample(newPosDist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        while (i < self.numParticles):
            self.particles.append(p[i % len(p)])
            i += 1
        ...
        for particle in self.particles:
            ...
            for ghost in range(self.numGhosts):
                if noisyDistances[ghost] == None:
                    ...
                    true_distance = util.manhattanDistance (pacmanPosition, particle[ghost])
                    temp *= emissionModels[ghost][true_distance]
