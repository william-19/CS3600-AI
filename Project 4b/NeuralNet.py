...
class Perceptron(object):
    ...
    def sigmoidDeriv(self, value):
        ...
        """
        """YOUR CODE"""
        return self.sigmoid(value) * (1 - self.sigmoid(value)) #from textbook page 727
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for index,weight in enumerate(self.weights):
            new_weight = weight + (alpha * delta * inActs[index])
            weights.append(new_weight)
            totalModification += abs(weight - new_weight)
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                perceptrons.append(perceptron.sigmoidActivation(current))
            result.append(perceptrons)
            current = perceptrons
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for i in range(len(nextLayer)):
                        error += nextLayer[i].weights[neuronNum + 1] * deltas[0][i]
                    delta = gPrime * error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for example in examplesTest:
        cond = True
        NN_output = nnet.feedForward(example[0])[-1]
        for i in range(len(NN_output)):
            if round(NN_output[i]) != example[1][i]:
                cond = False
        if cond == True:
            testCorrect += 1.0
        else:
    ...
    testAccuracy = float(testCorrect) / float(testError + testCorrect) 
