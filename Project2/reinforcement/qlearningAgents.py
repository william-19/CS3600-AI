...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        start_part = self.q_value[(state,action)]
        end_part = reward + (self.discount * self.computeValueFromQValues(nextState))
        update = ((1 - self.alpha) * start_part) + (self.alpha * end_part)
